...
public class CartServiceImpl implements CartService {
    ...
    public ResponseEntity<?> addProductToCart(AddProductInCartDto addProductInCartDto) {
        ...
        Optional<CartItems> optionalCartItems = cartItemsRepository.findByMedicamentsIdAndOrderIdAndUserId(addProductInCartDto.getProductId(), activeOrder.getId(), addProductInCartDto.getUserId());
        if (optionalCartItems.isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(null);
        } else {
            Optional<Medicaments> optionalProduct = productRepository.findById(addProductInCartDto.getProductId());
            Optional<User> optionalUser = userRepository.findById(addProductInCartDto.getUserId());
            if (optionalProduct.isPresent() && optionalUser.isPresent()) {
                CartItems cart = new CartItems();
                cart.setMedicaments(optionalProduct.get());
    ...
    public ResponseEntity<?> removeProductFromcart(Long productId, Long userId) {
        CartItems optionalCartItem = cartItemsRepository.findByMedicamentIdAndUserIdAndPendingOrderStatus(productId, userId);
    ...
    public OrderDto increaseProductQuantity(AddProductInCartDto addProductInCartDto) {
        Order activeOrder = orderRepository.findByUserIdAndOrderStatus(addProductInCartDto.getUserId(), OrderStatus.Pending);
        Optional<Medicaments> optionalProduct = productRepository.findById(addProductInCartDto.getProductId());
        Optional<CartItems> optionalCartItem = cartItemsRepository.findByMedicamentsIdAndOrderIdAndUserId(
                addProductInCartDto.getProductId(), activeOrder.getId(), addProductInCartDto.getUserId()
        );
        if (optionalProduct.isPresent() && optionalCartItem.isPresent()) {
            CartItems cartItem = optionalCartItem.get();
            Medicaments medicaments = optionalProduct.get();
            activeOrder.setAmount(activeOrder.getAmount() + medicaments.getPrice());
            activeOrder.setTotalAmount(activeOrder.getTotalAmount() + medicaments.getPrice());
